...
def despike_xyz_sparse(det_obj, radius=0.5, z_thresh=0.3, xy_thresh=0.3):
    """
    Corrects spikes in sparse 3D radar data using neighbor‐based weighted smoothing.
    Preserves genuine rotations since all points shift coherently.

    Args:
      det_obj (dict): {'x','y','z','numObj'} with numpy arrays
      radius (float): Neighborhood radius (meters)
      z_thresh (float): (unused, kept for signature compatibility)
      xy_thresh (float): (unused, kept for signature compatibility)
    ...
    for i, p in enumerate(pts):
        ...
        dists = np.linalg.norm(neigh - p, axis=1)
        # Gaussian weights
        w = np.exp(-(dists**2) / (2 * sigma**2))
        w /= np.sum(w)
        # Weighted average
        smooth_p = w @ neigh
        x_f[i], y_f[i], z_f[i] = smooth_p
    ...
    try:
        while True:
            ...
            if det_obj and det_obj.get('numObj',0)>=3:
                spr = despike_xyz_sparse(det_obj)
