...
def sophisticated_median_filter(arr: np.ndarray, window_size: int = 3) -> np.ndarray:
    """
    Mirror-pad then apply median in each sliding window to preserve edges.
    """
    half = window_size // 2
...
def soft_threshold(arr: np.ndarray, lam: float) -> np.ndarray:
    ...
    """
    return np.sign(arr) * np.maximum(np.abs(arr) - lam, 0.0)
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            # --- Integrated smoothing logic in your main processing block ---
            if det_obj is not None and det_obj.get('numObj', 0) >= 3:
                # Convert dict to point array first for smoothing
                x, y, z = det_obj['x'], det_obj['y'], det_obj['z']
                pts = np.vstack((x, y, z)).T

                # 1) Sophisticated median filtering on each coordinate
                x_med = sophisticated_median_filter(x, window_size=3)
                y_med = sophisticated_median_filter(y, window_size=3)
                z_med = sophisticated_median_filter(z, window_size=3)

                # 2) L1 sparse regularization via soft-thresholding
                lam = 0.05
                x_reg = soft_threshold(x_med, lam)
                y_reg = soft_threshold(y_med, lam)
                z_reg = soft_threshold(z_med, lam)

                # Reconstruct smoothed point array (if needed elsewhere)
                smoothed_pts = np.vstack((x_reg, y_reg, z_reg)).T
                ...
                # (velocity is unchanged, so omitted or reuse det_obj['velocity'] as needed)
