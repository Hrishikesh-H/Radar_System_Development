...
class PointCloudVisualizer(QtWidgets.QMainWindow):
    def __init__(self, pc: PointCloud, output_dir: str):
        ...
        self.label_offset = self.range_coords * 0.2  # Increased offset
    ...
    def _init_ui(self):
        ...
        self.combo_plane_axis.currentIndexChanged.connect(self._update_planes)
    ...
    def _init_gl(self):
        ...
        # Add axis labels at the ends of the axes with increased offset
        self.x_label = GLTextItem(
            pos=(self.max_coords[0] + self.range_coords[0] * 0.2, 0, 0), 
            text='X', 
            font=QtGui.QFont('Arial', 14), 
            color=(1,0,0,1)
        )
        self.y_label = GLTextItem(
            pos=(0, self.max_coords[1] + self.range_coords[1] * 0.2, 0), 
            text='Y', 
            font=QtGui.QFont('Arial', 14), 
            color=(0,1,0,1)
        )
        self.z_label = GLTextItem(
            pos=(0, 0, self.max_coords[2] + self.range_coords[2] * 0.2), 
            text='Z', 
            font=QtGui.QFont('Arial', 14), 
            color=(0,0,1,1)
        )
    ...
    def _init_plane_sliders(self):
        ...
        # Set slider ranges
        for slider in [self.slider_plane1, self.slider_plane2]:
            slider.setRange(0, 1000)
            slider.setValue(250 if slider == self.slider_plane1 else 750)
    ...
    def _handle_mouse_move(self, event):
        ...
        # Optimized hover detection using spatial partitioning
        screen_points = np.zeros((len(points), 2))
        for i, point in enumerate(points):
            screen_point = self._project_point(point)
            screen_points[i] = [screen_point.x(), screen_point.y()]
        
        # Calculate distances in bulk
        mouse_x, mouse_y = mouse_pos.x(), mouse_pos.y()
        dists = np.sqrt((screen_points[:,0] - mouse_x)**2 + (screen_points[:,1] - mouse_y)**2)
    ...
    def _update_planes(self):
        ...
        # Convert slider positions to actual values
        val1 = min_val + (self.slider_plane1.value() / 1000) * range_val
        val2 = min_val + (self.slider_plane2.value() / 1000) * range_val
        
        # Ensure plane1 <= plane2
        if val1 > val2:
            val1, val2 = val2, val1
            self.slider_plane1.setValue(int((val1 - min_val) / range_val * 1000))
            self.slider_plane2.setValue(int((val2 - min_val) / range_val * 1000))
    ...
    def _add_planes(self):
        """Create and add plane visualization to the view"""
        ...
        # Make planes very large to cover the entire view
        size = max(100, self.range_coords.max() * 2)  # Ensure coverage

        # Create two grid planes with bright colors
        for i, value in enumerate(self.plane_values):
            plane = GLGridItem()
            plane.setSize(x=size, y=size)
            plane.setSpacing(x=size/20, y=size/20)  # Dense grid for visibility

            # Position and orient the plane based on selected axis
            if axis == 0:  # X-axis
                plane.rotate(90, 0, 1, 0)
                plane.translate(value, 0, 0)
            elif axis == 1:  # Y-axis
                plane.rotate(90, 1, 0, 0)
                plane.translate(0, value, 0)
            else:  # Z-axis
                plane.translate(0, 0, value)

            # Set color - use the grid item's color property
            color = (1, 0, 0, 0.7) if i == 0 else (0, 0, 1, 0.7)
            plane.setColor(color)
            
            # Make sure planes are visible above other objects
            plane.setGLOptions('opaque')
    ...
    def _filter_points_between_planes(self):
        ...
        mask = (self.pc.coords[:, axis] >= min_val) & (self.pc.coords[:, axis] <= max_val)
        
        # Create a new scatter plot with just these points
        pts = self.pc.coords[mask]
        
        if pts.size == 0:
            self.status_bar.setText("No points found between planes")
            return
            
        # Create a new scatter plot item
        self.filtered_scatter = GLScatterPlotItem(
            pos=pts, 
            size=self.spin_size.value() * 1.5,
            color=(1, 1, 0, 0.8),  # Yellow highlight
            pxMode=True
        )
        self.view.addItem(self.filtered_scatter)
        
        # Show info in status bar
        self.status_bar.setText(
            f"Found {len(pts)} points between planes at {min_val:.2f} and {max_val:.2f}"
        )
    ...
    def _clear_filtered_points(self):
        ...
        if self.filtered_scatter is not None:
            self.view.removeItem(self.filtered_scatter)
            self.filtered_scatter = None
        self.status_bar.setText("Cleared point filter")
    ...
    def _reset_view(self):
        ...
        if self.initial_view:
            self.view.setCameraPosition(
                ...
                center=self.initial_view['center']
            )
