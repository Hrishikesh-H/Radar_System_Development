...
class PointCloudVisualizer(QtWidgets.QMainWindow):
    ...
    def _init_ui(self):
        ...
        btn_inf.clicked.connect(self._show_centroids_window)
    ...
    def _export_csv(self):
        """Export visible points to CSV file using file dialog"""
    ...
    def _save_image(self):
        """Save screenshot using file dialog"""
        # Open file dialog to choose save path
        file_dialog = QtWidgets.QFileDialog()
        file_dialog.setAcceptMode(QtWidgets.QFileDialog.AcceptSave)
    ...
    def _show_centroids_window(self):
        """Show a window with computed centroids and explanation"""
        mask = self.current_mask
        pts = self.pc.coords[mask]
        ...
        for c in np.unique(clusters):
            ...
            if len(sel) > 0:
                cen = sel.mean(axis=0)
                centroids.append((c, len(sel), cen))
        
        # Create dialog
        dialog = QtWidgets.QDialog(self)
        dialog.setWindowTitle('Cluster Centroids')
        layout = QtWidgets.QVBoxLayout(dialog)
        
        # Explanation label
        explanation = QtWidgets.QLabel(
            "A centroid is the mean position of all the points in a cluster. "
            "It represents the geometric center of the cluster and is calculated "
            "by averaging the coordinates of all points in the cluster. "
            "Below are the computed centroids for each cluster in the currently visible points."
        )
        explanation.setWordWrap(True)
        explanation.setStyleSheet("font-size: 12px; margin-bottom: 10px;")
        layout.addWidget(explanation)
        
        # Centroid table
        table = QtWidgets.QTableWidget()
        table.setColumnCount(5)
        table.setRowCount(len(centroids))
        table.setHorizontalHeaderLabels(['Cluster', 'Count', 'X', 'Y', 'Z'])
        table.setEditTriggers(QtWidgets.QAbstractItemView.NoEditTriggers)
        table.verticalHeader().setVisible(False)
        
        for i, (c, count, cen) in enumerate(centroids):
            table.setItem(i, 0, QtWidgets.QTableWidgetItem(str(c)))
            table.setItem(i, 1, QtWidgets.QTableWidgetItem(str(count)))
            table.setItem(i, 2, QtWidgets.QTableWidgetItem(f"{cen[0]:.2f}"))
            table.setItem(i, 3, QtWidgets.QTableWidgetItem(f"{cen[1]:.2f}"))
            table.setItem(i, 4, QtWidgets.QTableWidgetItem(f"{cen[2]:.2f}"))
...
if __name__ == '__main__':
    ...
    file_dialog.setNameFilter("CSV files (*.csv)")
    if file_dialog.exec_():
        input_csv = file_dialog.selectedFiles()[0]
    else:
        sys.exit(0)
    
    pc = PointCloud.from_csv(input_csv)
    viz = PointCloudVisualizer(pc)
    viz.run()
    sys.exit(app.exec_())