...
class PointCloudVisualizer(QtWidgets.QMainWindow):
    ...
    def _update_line_graphs(self, visible_points):
        ...
        # FIX: Use edges directly with stepMode=True
    ...
    def _add_planes(self):
        """Add planes for all enabled axes"""
        # Remove all existing planes
        for axis in ['x', 'y', 'z']:
            for i in range(2):
                if self.plane_systems[axis]['planes'][i] is not None:
                    try:
                        self.view.removeItem(self.plane_systems[axis]['planes'][i])
                    except Exception:
                        pass
                    self.plane_systems[axis]['planes'][i] = None
        ...
        for axis, system in self.plane_systems.items():
            ...
            axis_index = ['x', 'y', 'z'].index(axis)
            size = self.max_dimension * 2  # Use max_dimension for consistent size
            colors = [(1, 0.5, 0.5, 0.5), (0.5, 0.5, 1, 0.5)]  # More transparent
            
            for i, value in enumerate(system['values']):
                plane = GLGridItem()
                plane.setSize(x=size, y=size)
                plane.setSpacing(x=size/10, y=size/10)
                plane.setGLOptions('translucent')  # Important for visibility
