...
class LandingZoneAssessor:
    def __init__(self, distance_thresh=0.05, min_iterations=500, slope_thresh_deg=15, inlier_ratio_thresh=0.4):
...
# Assumes RadarDespiker and LandingZoneAssessor are defined/imported above
# from your_module import RadarDespiker, LandingZoneAssessor
...
def plot_plane_fit(x, y, z, coeffs, metrics, elev=25, azim=35,
    ...
    """
    fig = plt.figure(figsize=(45, 8), constrained_layout=True)
    ax = fig.add_subplot(111, projection='3d')
    plt.rcParams.update({'font.family': 'Helvetica', 'font.size': 15})
    ...
    # ------------------- TICKS WITH VARIABLE STEP ------------------- #
    ax.set_xticks(np.arange(np.floor(x_min), np.ceil(x_max) + step_x, step_x))
    ax.set_yticks(np.arange(np.floor(y_min), np.ceil(y_max) + step_y, step_y))
...
st.write(
    'Upload a CSV (columns: timestamp, x, y, z, snr, noise). App filters, buffers frames, fits planes, and displays metrics.')
...
if uploaded_file is not None:
    ...
    if not required.issubset(df.columns):
        ...
        # Guard: if no frames, stop (prevents slider error)
        if len(frames) == 0:
            st.warning("No buffer frames created with current buffer length / overlap. Adjust sliders.")
            st.stop()
        ...
        with col2:
            playback_speed = st.number_input("Playback Speed (frames/sec)", min_value=0.5, max_value=10.0, value=1.0,
                                             step=0.5)

        frame_idx = st.slider('Select Frame Index', min_value=0, max_value=len(frames) - 1,
                              value=st.session_state.frame_idx)
        st.session_state.frame_idx = frame_idx

        # instantiate modules (kept as original logic)
        ...
        # Display metrics in terminal style
        st.markdown("<h3>Landing Zone Assessment Metrics</h3>", unsafe_allow_html=True)

        # Determine status text color
        status_color = "green" if safe else "#f56767"  # Orange/red for dark mode visibility
        status_text = f"<b style='color:{status_color};'>SAFE</b>" if safe else f"<b style='color:{status_color};'>UNSAFE</b>"
        ...
        ">
        <span style='color:#50fa7b;'>AIRL@Hiremath</span>:<span style='color:#8be9fd;'>~</span>$ Landing Zone Assessment Metrics

        <span style='color:#8be9fd;'>▶ STATUS</span><br>
        &nbsp;&nbsp;Landing Status       : {status_text}<br>

        <span style='color:#8be9fd;'>▶ METRICS</span><br>
        &nbsp;&nbsp;Slope w.r.t Y (deg)  : <b style='color:gold;'>{metrics.get('slope_deg', float('nan')):.2f}</b><br>
        &nbsp;&nbsp;Confidence Score     : <b style='color:gold;'>{metrics.get('confidence_score', float('nan')):.2f}</b><br>
        &nbsp;&nbsp;Inlier Ratio         : <b style='color:gold;'>{metrics.get('inlier_ratio', float('nan')):.2f}</b><br>
        &nbsp;&nbsp;Surface Quality      : <b style='color:gold;'>{metrics.get('surface_quality', float('nan')):.2f}</b><br>
        &nbsp;&nbsp;Mean Residual        : <b style='color:gold;'>{metrics.get('mean_residual', float('nan')):.4f}</b><br>
        """

        # Reason if unsafe
        if not safe and metrics.get('reason', ''):
            html_content += f"&nbsp;&nbsp;Reason               : <b style='color:#ffa500;'>{metrics['reason']}</b><br>"

        # ----------------- Previous frames summary ------------------ #
        prev_n = 5
        start_idx = max(0, frame_idx - prev_n)
        prev_frames = frames[start_idx:frame_idx]

        if len(prev_frames) == 0:
            prev_status_line = "No previous frames available"
            safe_count = 0
        else:
            status_list = []
            safe_count = 0
            for i, pf in enumerate(prev_frames, start=start_idx):
                det_obj_pf = {
                    'x': pf['x'].to_numpy(),
                    'y': pf['y'].to_numpy(),
                    'z': pf['z'].to_numpy(),
                ...
                snr_pf = pf['snr'].to_numpy()
                noise_pf = pf['noise'].to_numpy()
                filtered_pf = radar_despiker.process(det_obj_pf, snr_pf, noise_pf)
                safe_pf, _ = lz_assessor.assess(filtered_pf)
                if safe_pf:
                    safe_count += 1
                    status_text_pf = "<b style='color:green;'>SAFE]</b>"
                else:
                    status_text_pf = "<b style='color:red;'>UNSAFE]</b>"
                status_list.append(f"[F{i}:{status_text_pf}")
            prev_status_line = "; ".join(status_list)

        total_frames = len(status_list)
        if total_frames > 0:
            if safe_count > total_frames / 2:
                final_line = "<b style='color:orange;'>Info: </b>" + "<b style='color:green;'>SAFE TO LAND</b>"
            else:
                final_line = "<b style='color:orange;'>Warning: </b>" + "<b style='color:red;'>INCONSISTENT</b>"
        else:
            final_line = "<i>No previous frame data to evaluate final SAFE status</i>"

        html_content += f"<br><span style='color:#8be9fd;'>▶ HISTORY</span><br>&nbsp;&nbsp;{prev_status_line}<br>&nbsp;&nbsp;{final_line}<br></div>"

        # Append new block to terminal history (keep stacking)
        st.session_state.terminal_history += html_content

        # 🧹 Clear Terminal Button (addition)
        if st.button("🧹 Clear Terminal"):
            st.session_state.terminal_history = ""

        # Display stacked terminal history
        st.markdown(st.session_state.terminal_history, unsafe_allow_html=True)

        # Sidebar sliders (axis steps)
        ...
        elev = st.slider('3D Elevation Angle (deg)', min_value=0, max_value=180, value=25, step=1)
        azim = st.slider('3D Azimuth Angle (deg)', min_value=0, max_value=360, value=35, step=1)

        # Ensure metrics contains plane before plotting (minimal guard to avoid crash)
        if 'plane' not in metrics or metrics['plane'] is None:
            st.error("Plane not found in metrics. Cannot plot best-fit plane.")
        else:
