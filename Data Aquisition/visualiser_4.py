...
class PointCloudVisualizer(QtWidgets.QMainWindow):
    ...
    def _init_gl(self):
        ...
        # Add axis labels at the ends of the axes with reduced offset
        # Enable additive rendering to ensure labels are always visible
        self.x_label = GLTextItem(
            pos=(self.max_coords[0] + self.range_coords[0] * 0.05, 0, 0), 
            text='X', 
        ...
        self.y_label = GLTextItem(
            pos=(0, self.max_coords[1] + self.range_coords[1] * 0.05, 0), 
            text='Y', 
            font=QtGui.QFont('Arial', 36), 
            color=(0,1,0,1))
        ...
        self.z_label = GLTextItem(
            pos=(0, 0, self.max_coords[2] + self.range_coords[2] * 0.05), 
            text='Z', 
            font=QtGui.QFont('Arial', 36), 
            color=(0,0,1,1))
    ...
    def _add_planes(self):
        ...
        size = max(100, self.range_coords.max() * 2)

        # Recreate planes from scratch
        for i, value in enumerate(self.plane_values):
            plane = GLGridItem()
            plane.setSize(x=size, y=size)
            plane.setSpacing(x=size/10, y=size/10)
            plane.setGLOptions('translucent')
            # No need for resetTransform() since this is a new instance
            ...
            # Bright colors with full alpha
            color = (1, 0, 0, 1.0) if i == 0 else (0, 0, 1, 1.0)
    ...
    def _reset_view(self):
        ...
        if self.initial_view:
            self.view.setCameraPosition(
                pos=self.initial_view['center'],  # Changed from 'center' to 'pos'
