...
class PointCloudVisualizer(QtWidgets.QMainWindow):
    ...
    def _init_gl(self):
        ...
        # Position labels at the corners of the bounding box
    ...
    def _add_planes(self):
        ...
        # Calculate appropriate plane size based on data dimensions
        if axis == 0:  # X-axis plane
            size_y = self.max_coords[1] - self.min_coords[1]
            size_z = self.max_coords[2] - self.min_coords[2]
            size = max(size_y, size_z) * 1.2
        elif axis == 1:  # Y-axis plane
            size_x = self.max_coords[0] - self.min_coords[0]
            size_z = self.max_coords[2] - self.min_coords[2]
            size = max(size_x, size_z) * 1.2
        else:  # Z-axis plane
            size_x = self.max_coords[0] - self.min_coords[0]
            size_y = self.max_coords[1] - self.min_coords[1]
            size = max(size_x, size_y) * 1.2
        ...
        for i, value in enumerate(self.plane_values):
            ...
            plane.setGLOptions('additive')  # Changed to additive for better visibility
            plane.resetTransform()  # Reset transform state

            if axis == 0:  # X-axis plane
                plane.rotate(90, 0, 1, 0)  # Rotate to YZ plane
                plane.translate(
                    value, 
                    (self.min_coords[1] + self.max_coords[1]) / 2,
                    (self.min_coords[2] + self.max_coords[2]) / 2
                )
            elif axis == 1:  # Y-axis plane
                plane.rotate(90, 1, 0, 0)  # Rotate to XZ plane
                plane.translate(
                    (self.min_coords[0] + self.max_coords[0]) / 2,
                    value,
                    (self.min_coords[2] + self.max_coords[2]) / 2
                )
            else:  # Z-axis plane
            ...
            color = (1, 0.5, 0.5, 1.0) if i == 0 else (0.5, 0.5, 1, 1.0)  # Brighter colors
