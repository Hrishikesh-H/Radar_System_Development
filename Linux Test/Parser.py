...
class RadarParser:
    ...
    def initialize_ports(self):
        ...
        try:
            settings = f"CLI port settings: {self.cli_serial.portstr} -> {self.cli_serial.baudrate}, " \
                       ...
                       f"rtscts={self.cli_serial.rtscts}\n" \
                       f"Data port settings: {self.data_serial.portstr} -> {self.data_serial.baudrate}, " \
    ...
    def read_frame(self):
        """
        Attempt to read one complete radar frame. If no data arrives, or not enough data
        for a full packet, return (None, None, None, None). Otherwise, return (header, det_obj, snr, noise).
        """
        try:
            ...
            # Show raw bytes read (length and hex) if debug
            if data:
                ...
                # No data arrived this iteration
                if self.debug:
        ...
        # If we appended data, attempt to locate a full packet
        if self.buffer:
            while True:
                idx = self.find_magic_word(self.buffer)
                if idx == -1:
                    # No magic word found—if buffer grows too big, trim it
                    if len(self.buffer) > 8192:
                        print(f"[{self._timestamp()}] [DEBUG] Buffer >8192 bytes without magic word, trimming")
                        self.buffer = self.buffer[-1024:]
                    break

                # If magic word isn't at index 0, discard preceding bytes
                if idx > 0:
                    if self.debug:
    ...
    def close(self):
        """
        Close both serial ports (if open) and log final message.
        """
        try:
