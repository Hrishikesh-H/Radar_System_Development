...
# ============================ managers.py ============================
def radar_manager(finder, shared, stop_event):
    """ThreadPool I/O-bound radar reconnect/init."""
    while not stop_event.is_set():
        now = time.time()
...
def autopilot_manager(finder, shared, stop_event, io_executor):
    """ThreadPool I/O-bound autopilot reconnect/setup."""
    while not stop_event.is_set():
        if shared.get('compensator') is None:
            try:
                master, _ = finder.find_autopilot_connection(timeout=2.0, exclude_ports=[shared.get('cli'), shared.get('data')])
            except RuntimeError:
                master = None
            if master:
                ...
                io_executor.submit(mode_watcher_loop, master, shared['mode_holder'], shared['stop_mode'])
...
if __name__ == '__main__':
    ...
    shared = {'radar': None, 'last_radar_time': 0.0, 'cli': None, 'data': None,
              'compensator': None, 'mode_holder': None, 'landing_monitor': None}
    ...
    io_executor.submit(radar_manager, finder, shared, stop_radar)
    ...
    try:
        while True:
            radar = shared.get('radar')
            if not radar:
                if stop_radar.is_set() or stop_auto.is_set():
                    break
                time.sleep(0.1)
                continue

            # I/O: read raw frame
            io_future = io_executor.submit(radar.read_frame)
            try:
                header, det_obj, snr, noise = io_future.result(timeout=5.0)
            except Exception as e:
            ...
            # CPU-bound: despike, assess, transform
            ...
            if despike_res.get('numObj', 0) >= 3:
                ...
                if comp:
                    transform_f = cpu_executor.submit(comp.transform_pointcloud, pts)
                    try:
                        pts_enu = transform_f.result()
                    except Exception:
                        comp.close()
                    ...
                    pts_enu = pts
                smoothed = {'x': pts_enu[:,0],'y':pts_enu[:,1],'z':pts_enu[:,2],'numObj':despike_res['numObj']}

            # Output status
            if not assess_res.get('plane'):
                radar.warn_print('Landing zone UNSAFE: Insufficient data')
            else:
                msg = (f"SAFE slope={assess_res['slope_deg']:.1f}deg" if safe else
                       f"UNSAFE slope={assess_res['slope_deg']:.1f}deg")
        ...
        stop_radar.set()
        stop_auto.set()
        # allow loops to exit promptly
        io_executor.shutdown(wait=True)
        # terminate worker processes explicitly
        cpu_executor.shutdown(wait=False)
        for p in cpu_executor._processes.values():
            p.terminate()
        cpu_executor.shutdown(wait=True, cancel_futures=True)
        if shared.get('radar'):
            try: shared['radar'].close()
            except: pass
        if shared.get('compensator'):
            try: shared['compensator'].close()
            except: pass
        sys.exit(0)
